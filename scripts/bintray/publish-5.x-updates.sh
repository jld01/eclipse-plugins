...
RELEASE=5.1.1
ARCHIVE_NAME=ilg.gnumcueclipse.repository-${RELEASE}-202007271621.zip

(
  curl -L -v https://download.eclipse.org/embed-cdt/releases/5.1.1/${ARCHIVE_NAME} -o ilg.gnumcueclipse.repository-${RELEASE}-SNAPSHOT.zip
...
FULL_VERSION=${RELEASE}-202007271621 # "$(grep "unit id='ilg.gnumcueclipse.core'" targetPlatformRepository/content.xml | sed "s/.*version='\(.*\)'.*/\1/")"
